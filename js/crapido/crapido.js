$(document).ready(function(){
	var $newCreavcr = $('#add_votoscr');
	var $btnCreavcr = $('#btn_votoscr');
	...
	$btnCreavr.on('click',function(){
		...
		$('#seccion,#votos,#casilla, #vpartido1,#vpartido2,#vpartido3, #mensajes').val('');
		$('#mensajes').hide();
		$('#mensajes0').hide();
	...
	$selelec.on('change',function(){
		...
		var url_refresca = $('#baseUrl').val()+'crapido/capturavotoscr/refresca_capturacr';			//PARA REFRESCAR POR AJAX LA TABLA
	    $.post( url_refresca, datose)
		  .done(function( data ) {
		  	$('#datos_votoscr').html( data );
		  });
    })
